...
namespace OpenRealEstate.Transmorgrifiers.Json
{
    ...
    public class ListingConverter : JsonCreationConverter<Listing>
    {
        ...
        protected override Listing Create(Type objectType, JObject jObject)
        {
            ...
            var doesValueExist = jObject.TryGetValue(propertyName, StringComparison.OrdinalIgnoreCase, out var value);
            if (!doesValueExist ||
            ...
            if (value.ToString().Equals("Residential", StringComparison.OrdinalIgnoreCase))
            {
                return new ResidentialListing();
            }

            if (value.ToString().Equals("Rental", StringComparison.OrdinalIgnoreCase))
            {
                return new RentalListing();
            }

            if (value.ToString().Equals("Land", StringComparison.OrdinalIgnoreCase))
            {
                return new LandListing();
            }

            if (value.ToString().Equals("Rural", StringComparison.OrdinalIgnoreCase))
            {
